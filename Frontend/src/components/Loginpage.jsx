  ...
  return (
    <div className={` w-full h-[92vh] flex flex-col items-center justify-between `}>
        ...
        <div className={` w-full h-[80%] flex flex-col items-center justify-center `}>
            <h1 className={`text-center text-xl font-serif font-medium md:text-4xl md:pb-5 lg:text-5xl xl:text-xl xl:pt-42`}>Login <i className="ri-login-box-line"></i> </h1>
            ...
            <form className={`mt-3 flex flex-col gap-2 items-center justify-center`} onSubmit={handleSubmit} action="#" method='post'>
                ...
                <input className={`border outline-0 border-gray-500 w-64 py-2 px-2 text-sm mx-auto rounded-sm md:w-[27rem] md:py-4 md:px-2.5 md:text-xl lg:w-[32rem] lg:py-5 lg:text-xl xl:w-80 xl:py-2 xl:text-sm xl:px-2`} type="password" value={password} onChange={(e)=>setPassword(e.target.value)} placeholder='Password' required />
                 ...
                 <div className={` mt-1 text-xs flex items-center justify-between gap-14 md:gap-32 md:py-1.5 lg:gap-42 xl:gap-[80px] xl:mt-0`}>
                    ...
                    <Link to='/' className={`cursor-pointer hover:text-black text-gray-600 md:text-lg lg:text-xl xl:text-sm`} >Create account</Link>
                 </div>

                 <button className={`bg-black/80 mb-2 hover:bg-black text-white px-6 text-sm py-2 mt-3 cursor-pointer rounded md:text-2xl md:py-4 md:mt-7 md:px-16 xl:text-sm xl:py-1.5 xl:px-7 xl:mt-3`}type="submit">Sign in</button>
            </form>
        </div>
